...
func (r *TodoListPostgres) Update(userId int, listId int, input todo.UpdateListInput) error {
	...
	querty := fmt.Sprintf("UPDATE %s tl SET %s FROM %s ul WHERE tl.id = ul.list_id AND ul.list_id = $%d AND ul.user_id = $%d",
...
func (r *TodoListPostgres) Delete(userId, listId int) error {
	...
	if rowsAffected == 0 {
		return fmt.Errorf("list with id=%d not found or does not belong to user with id=%d", listId, userId)
	}
